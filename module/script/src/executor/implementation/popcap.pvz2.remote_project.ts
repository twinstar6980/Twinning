namespace Twinning.Script.Executor.Implementation.Popcap.Pvz2.RemoteProject {

	// #region interface

	export function activate(
	): void {
		push_typical_method('popcap.pvz2.remote_project', [
			typical_method({
				identifier: 'execute',
				...
				argument: [
					...
					typical_argument_string({
						identifier: 'action',
						option: Support.Popcap.Pvz2.RemoteProject.ActionE,
						checker: null,
						automatic: null,
						condition: null,
					}),
					typical_argument_string({
						identifier: 'target',
						option: Support.Popcap.Pvz2.RemoteProject.TargetE,
						checker: null,
						automatic: null,
				...
				worker: ({project_directory, action, target, rton_version_number, rton_version_native_string_encoding_use_utf8, rton_encode_buffer_size}, store: {}) => {
					Support.Popcap.Pvz2.RemoteProject.execute(project_directory, action as any, target === '' ? null : target as any, {number: rton_version_number as any, native_string_encoding_use_utf8: rton_version_native_string_encoding_use_utf8}, rton_encode_buffer_size.value());
					return;
				},
			}),
		]);
