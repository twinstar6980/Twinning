...
export module twinning.kernel.tool.texture.transformation.tiling.encode;
...
export namespace Twinning::Kernel::Tool::Texture::Transformation::Tiling {
	...
	struct Encode :
		...
		inline static auto process_image(
			...
			assert_test(Math::is_padded_size(raw.size().width, tile_size.width) && Math::is_padded_size(raw.size().height, tile_size.height));
