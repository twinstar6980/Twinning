...
* [`data.cppm`](../module/shell/shell/bridge/data.cppm) ：声明 `Kernel` 中的数据类型。
  ...
  > `Kernel` 模块中已经提供了适用于 `C++` 的接口声明 [`interface.hpp`](../module/kernel/kernel/interface/interface.hpp) ，接口需要用户传递 C 式的结构体作为参数值与返回值。
...
* [`client.cppm`](../module/shell/shell/bridge/client.cppm) ：定义 `Shell` 客户端的抽象类。

  > [`main_console.cppm`](../module/shell/shell/main_console.cppm) ：实现了对基本的命令行式 `Shell` 客户端的封装。
...
用户可以自行重写主脚本的运行逻辑，但更推荐在已有的 [Script 模块](../module/script) 基础上扩展开发，它已为用户提供了丰富的功能与良好的交互机制。
...
内核接口具有严格的类型限制，因此，在开发层面，应使用 `TypeScript` 作为开发语言，并编译为 `JavaScript` 供工具使用。`Script` 模块中的 [`kernel.d.ts`](../module/script/src/kernel.d.ts) 声明了 `Kernel` 所定义的接口。
