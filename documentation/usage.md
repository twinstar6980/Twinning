...
* 最后，匹配携带某些语言的可执行文件扩展名的文件（按优先级依次为）`<alias>.py` 、`<alias>.jar` 。

对于检索到的目标程序，将按照以下规则执行：

* 如果目标程序是可执行文件，将直接创建对应进程。

* 如果目标程序是脚本文件或某些语言的可执行文件，将调用对应的解释器执行它们，解释器程序也会通过上述流程进行检索，但只会匹配原生可执行文件，不会尝试二次解释。

  > 对于 `sh` 脚本，将调用 `sh` 解释器执行。\
  > 对于 `ps1` 脚本，将调用 `pwsh` 解释器执行。\
  > 对于 `cmd` 或 `bat` 脚本，将调用 `cmd` 解释器执行。\
  > 对于 `py` 文件，将调用 `python` 解释器执行。\
  > 对于 `jar` 文件，将调用 `java` 解释器执行。
