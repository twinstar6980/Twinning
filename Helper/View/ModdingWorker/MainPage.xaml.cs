...
namespace Helper.View.ModdingWorker {
	...
	public class MainPageController : CustomController {
		...
		public async Task<String?> LaunchSession (
			...
			var argument = new List<String>([..Setting.Data.ModdingWorker.Argument, ..this.AdditionalArgument]);
			var temporaryKernel = StorageHelper.Temporary();
			var library = new Bridge.Library();
			this.SessionTask = new (() => (Bridge.Launcher.Launch(this.SessionHost, library, script, argument)));
			this.NotifyPropertyChanged(
			...
			if (exception is null) {
				await this.PublishOutput(MessageType.Success, "SUCCEEDED", [result.AsNotNull()]);
			}
			else {
				await this.PublishOutput(MessageType.Error, "FAILED", [exception.ToString()]);
			}
		...
		public async Task PublishOutput (
			...
			this.uOutputList_ItemsSource.Add(new () {
				Host = this,
				Type = type,
				Title = title,
		...
		public ObservableCollection<MainPageOutputItemController> uOutputList_ItemsSource { get; } = [];
		...
		public InputValue uInput_Value { get; } = new () { Data = null };
			...
			Task.WaitAll(this.Controller.View.DispatcherQueue.EnqueueAsync(async () => {
				switch (method) {
					...
					case "output_message": {
						...
						await this.Controller.PublishOutput(
							type switch {
					...
					case "input_pause": {
						GF.AssertTest(argument.Count == 1);
						await this.Controller.RequestInput(InputType.Pause, null);
						break;
					}
					case "input_boolean": {
						GF.AssertTest(argument.Count == 1);
						var input = (await this.Controller.RequestInput(InputType.Boolean, null)).AsStructOrNull<Boolean>();
						result.Add(input is null ? "" : !input.Value ? "n" : "y");
						break;
					}
					case "input_integer": {
						GF.AssertTest(argument.Count == 1);
						var input = (await this.Controller.RequestInput(InputType.Integer, null)).AsStructOrNull<Integer>();
						result.Add(input is null ? "" : input.Value == 0 ? "0" : (input.Value < 0 ? $"{input}" : $"+{input}"));
						break;
					}
					case "input_floater": {
						GF.AssertTest(argument.Count == 1);
						var input = (await this.Controller.RequestInput(InputType.Floater, null)).AsStructOrNull<Floater>();
					...
					case "input_size": {
						GF.AssertTest(argument.Count == 1);
						var input = (await this.Controller.RequestInput(InputType.Size, null)).AsClassOrNull<SizeExpression>();
						result.Add(input is null ? "" : input.ToString());
						break;
					}
					case "input_string": {
						GF.AssertTest(argument.Count == 1);
						var input = (await this.Controller.RequestInput(InputType.String, null)).AsClassOrNull<String>();
						result.Add(input is null ? "" : input);
						break;
					}
					case "input_path": {
						GF.AssertTest(argument.Count == 1);
						var input = (await this.Controller.RequestInput(InputType.Path, null)).AsClassOrNull<PathExpression>();
						result.Add(input is null ? "" : input.ToString());
						break;
					}
					case "input_enumeration": {
						...
						var input = (await this.Controller.RequestInput(InputType.Enumeration, option)).AsClassOrNull<EnumerationExpression>();
						result.Add(input is null ? "" : input.ToString());
						break;
					}
					case "pick_path": {
						...
						var selection = type switch {
							"file"      => await StorageHelper.PickFile(WindowHelper.Find(this.Controller.View)),
							"directory" => await StorageHelper.PickDirectory(WindowHelper.Find(this.Controller.View)),
							_           => throw new (),
						};
