...
namespace Helper.View.AnimationViewer {
	...
	public class MainPageController : CustomController {
		...
		public void Initialize (
			...
			this.ImageFilterRule = Setting.Data.AnimationViewer.ImageFilterRule;
		...
		public async Task Load (
			...
			try {
				...
				App.MainWindow.PublishTip(InfoBarSeverity.Error, "Failed to load animation.", e.ToString());
				return;
			}
			...
			this.NotifyPropertyChanged(
				...
				nameof(this.uImageDirectorySourcePick_IsEnabled),
		...
		public async Task ApplyLoad (
			...
			if (this.Loaded) {
				...
				await App.Instance.AppendRecentJumperItem(new () {
		...
		public async Task ApplyFilterRule (
			...
			await this.ApplyFilter(
				this.Animation.Image.Select((value) => (this.ImageFilterRule.Length != 0 && Regex.IsMatch(AnimationHelper.ParseImageFileName(value.Name), this.ImageFilterRule) ? false : (Boolean?)null)).ToList(),
		...
		public async void uPage_Drop (
			...
			if (args.DataView.Contains(StandardDataFormats.StorageItems)) {
				...
				if (item.Count != 1) {
					App.MainWindow.PublishTip(InfoBarSeverity.Error, "Source is multiply.", "");
					return;
				}
				var animationFile = StorageHelper.Regularize(item[0].Path);
				if (!StorageHelper.ExistFile(animationFile)) {
					App.MainWindow.PublishTip(InfoBarSeverity.Error, "Source is not a file.", "");
					return;
				}
				var imageDirectory = StorageHelper.Parent(animationFile).AsNotNull();
		...
		public async void uAnimationFileSourcePick_Click (
			Object          sender,
			RoutedEventArgs args
		) {
			var senders = sender.AsClass<Button>();
			...
			var animationFile = await StorageHelper.PickFile(WindowHelper.Find(this.View), "json");
		...
		public Boolean uImageDirectorySourcePick_IsEnabled {
			get {
				if (!this.Loaded) { return false; }
				return true;
			}
		...
		public async void uImageDirectorySourcePick_Click (
			Object          sender,
			RoutedEventArgs args
		) {
			var senders = sender.AsClass<Button>();
			...
			var imageDirectory = await StorageHelper.PickDirectory(WindowHelper.Find(this.View));
		...
		public List<MainPageMainSpriteItemController> uMainSpriteList_ItemsSource {
			get {
				if (!this.Loaded) { return [new () { Host = this, Index = null }]; }
	...
	public class MainPageImageItemController : CustomController {
		...
		public String uTitle_Text {
			get {
				...
				return AnimationHelper.ParseImageFileName(model.Name);
			}
		}
