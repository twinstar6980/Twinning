...
namespace Helper.View.ModuleLauncher {
	...
	public class MainPageController : CustomController {
		...
		public async void uAddPinnedJumperItem_Click (
			...
			Setting.Data.ModuleLauncher.PinnedJumperConfiguration.Add(new () {
				Title = "Untitled",
				ModuleType = ModuleType.ModuleLauncher,
				ModuleOption = [],
				WindowOption = [],
			});
			this.uPinnedJumperList_ItemsSource.Add(new () { Host = this, Configuration = Setting.Data.ModuleLauncher.PinnedJumperConfiguration[^1] });
			await Setting.Save();
			return;
		}
	...
	public class MainPageJumperItemController : CustomController {
		...
		public async void uOpenAsTab_Click (
			...
			var senders = sender.AsClass<Button>();
			await JumperConfiguration.Launch(this.Configuration, false);
			return;
		}
		...
		public async void uOpenAsWindow_Click (
			...
			var senders = sender.AsClass<Button>();
			await JumperConfiguration.Launch(this.Configuration, true);
			return;
		}
		...
		public async void uEdit_Click (
			...
			var senders = sender.AsClass<Button>();
			await ControlHelper.ShowDialogFixed(this.Host.View, "Jumper Configuration", new JumperConfigurationPanel() {
		...
		public async void uRecentPin_Click (
			...
			this.Host.uPinnedJumperList_ItemsSource.Add(new () { Host = this.Host, Configuration = this.Configuration });
			this.Host.uRecentJumperList_ItemsSource.Remove(this);
			await Setting.Save();
			return;
		}
		...
		public async void uModuleSetting_Click (
			...
			var senders = sender.AsClass<Button>();
			await ControlHelper.ShowDialogFixed(this.Host.View, "Module Setting", this.Configuration.ModuleType switch {
