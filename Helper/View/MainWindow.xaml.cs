...
namespace Helper.View {
	...
	public class MainWindowController : CustomController {
		...
		public async Task InsertTabItem (
			...
			frame.NavigateToType(model.Page, option, new () { IsNavigationStackEnabled = false, TransitionInfoOverride = new DrillInNavigationTransitionInfo() });
			this.uTab_TabItemsSource.Add(new () { Host = this, Model = model, Frame = frame });
			await Task.Delay(40);
	...
	public class MainWindowTabItemController : CustomController {
		...
		public FontIconSource uRoot_IconSource {
			get {
				return new () { Glyph = this.Model.Icon };
			}
		}
