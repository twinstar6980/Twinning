/** 图集打包 */
namespace TwinKleS.Support.Atlas.Pack {
	...
	export function pack_fsh(
		...
		for (let sprite_name in manifest.sprite) {
			...
			CoreX.Image.File.png_read_file(`${sprite_directory}/${sprite_name}.png`, atlas.sub(Core.Image.ImagePosition.value(e.position), Core.Image.ImageSize.value(e.size)));
		}
		return;
	}
	...
	export function unpack_fsh(
		...
		for (let sprite_name in manifest.sprite) {
			let e = manifest.sprite[sprite_name];
			CoreX.Image.File.png_write_file(`${sprite_directory}/${sprite_name}.png`, atlas.sub(Core.Image.ImagePosition.value(e.position), Core.Image.ImageSize.value(e.size)));
		}
		return;
	}
	...
	export function pack_auto_fsh(
		...
		let sprite_box = record_from_array(sprite_file_list, (i, e) => {
			let size = CoreX.Image.File.png_size_file(`${sprite_directory}/${e}.png`);
		...
		for (let sprite_file in sprite_rect) {
			...
			CoreX.Image.File.png_read_file(`${sprite_directory}/${sprite_file}.png`, sprite);
		}
	...
	export function pack_fs(
		...
		CoreX.Image.File.png_write_file(atlas_file, atlas_view);
		return;
	}
	...
	export function unpack_fs(
		...
		let manifest = CoreX.JSON.read_fs_js<Manifest>(manifest_file);
		let atlas = CoreX.Image.File.png_read_file_of(atlas_file);
		let atlas_view = atlas.view();
	...
	export function pack_auto_fs(
		...
		CoreX.Image.File.png_write_file(atlas_file, atlas.view());
		return;
	}
