namespace TwinStar.Script.Support.PopCap.RTON.DecodeLenient {
		...
		switch (type_identifier) {
			...
			case 0x82n: {
				...
				if (actual_size.value !== Number(size)) {
					Console.notify('w', `data:${data.p().toString(16)}h : invalid utf-8 string size : except ${size} but actual ${actual_size.value}`, []);
				}
				value = content;
				break;
			}
				...
				switch (data.u8()) {
					...
					case 0x02n: {
						...
						if (sheet_actual_size.value !== Number(sheet_size)) {
							Console.notify('w', `data:${data.p().toString(16)}h : invalid utf-8 string size : except ${sheet_size} but actual ${sheet_actual_size.value}`, []);
						}
					...
					case 0x03n: {
						...
						if (alias_actual_size.value !== Number(alias_size)) {
							Console.notify('w', `data:${data.p().toString(16)}h : invalid utf-8 string size : except ${alias_size} but actual ${alias_actual_size.value}`, []);
						}
			...
			case 0x86n: {
				...
				if (value.length !== Number(size)) {
					Console.notify('w', `data:${data.p().toString(16)}h : invalid array size : except ${size} but actual ${value.length}`, []);
				}
				break;
			}
			case 0x85n: {
	...
	export function decode_whole(
		...
		if (data.u32() !== 0x4E4F5452n) {
			Console.notify('w', `data:${data.p().toString(16)}h : invalid magic`, []);
		}
