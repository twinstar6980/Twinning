namespace TwinStar.Script.Support.PvZ2.PackageProject.Transpile {
	...
	function transpile_resource(
		...
		switch (resource_setting.type) {
			...
			case 'general': {
				...
				if (option.specialize_rton !== null && resource_property.path.toLowerCase().endsWith('.rton')) {
					Console.warning(`Transpiling resource by specialize_rton ...`, [`${package_setting.name}:${part_name}/${group_name}/${resource_name}`]);
					let new_source_file = make_scope_child_path(resource_directory, 'source.json');
					KernelX.Tool.PopCap.ReflectionObjectNotation.decode_cipher_fs(source_file, new_source_file, option.specialize_rton.version, option.specialize_rton.crypt === null ? null : option.specialize_rton.crypt.key);
					KernelX.FileSystem.remove(source_file);
					resource_setting.type = 'special_rton';
					resource_setting.property = {
						conversion: `adaptive:default`,
						path: resource_property.path.slice(0, -5),
					} as SpecialRTONResourceProperty;
				}
				if (option.specialize_pam !== null && resource_property.path.toLowerCase().endsWith('.pam')) {
					Console.warning(`Transpiling resource by specialize_pam ...`, [`${package_setting.name}:${part_name}/${group_name}/${resource_name}`]);
					let new_source_file = make_scope_child_path(resource_directory, 'source.json');
					KernelX.Tool.PopCap.Animation.decode_fs(source_file, new_source_file, option.specialize_pam.version);
					KernelX.FileSystem.remove(source_file);
					resource_setting.type = 'special_pam';
					resource_setting.property = {
						conversion: `basic:${option.specialize_pam.version.number}`,
						path: resource_property.path.slice(0, -4),
					} as SpecialPAMResourceProperty;
				}
				if (option.specialize_wem !== null && resource_property.path.toLowerCase().endsWith('.wem')) {
					Console.warning(`Transpiling resource by specialize_wem ...`, [`${package_setting.name}:${part_name}/${group_name}/${resource_name}`]);
					let new_source_file = make_scope_child_path(resource_directory, 'source.wav');
					Support.Wwise.Media.Decode.decode_fs(source_file, new_source_file);
					KernelX.FileSystem.remove(source_file);
					resource_setting.type = 'special_wem';
					resource_setting.property = {
						conversion: `adaptive:compressed`,
						path: resource_property.path.slice(0, -4),
			...
			case 'texture': {
				...
				if (option.specialize_ptx !== null) {
					Console.warning(`Transpiling resource by specialize_ptx ...`, [`${package_setting.name}:${part_name}/${group_name}/${resource_name}`]);
					...
					let format = option.specialize_ptx.format.find((value) => (value.index === resource_property.format));
					assert_test(format !== undefined, `unknown texture format : ${resource_property.format}`);
					Support.PopCap.Texture.Encoding.decode(data_stream, image_view, format.format);
			...
			case 'special_rton': {
				...
				if (option.generalize_rton !== null) {
					Console.warning(`Transpiling resource by generalize_rton ...`, [`${package_setting.name}:${part_name}/${group_name}/${resource_name}`]);
			...
			case 'special_ptx': {
				...
				if (option.generalize_ptx !== null) {
					Console.warning(`Transpiling resource by generalize_ptx ...`, [`${package_setting.name}:${part_name}/${group_name}/${resource_name}`]);
			...
			case 'special_pam': {
				...
				assert_test(conversion_setting !== null, `invalid conversion name`);
				let source_file = make_scope_child_path(resource_directory, 'source.json');
				if (option.generalize_pam !== null) {
					Console.warning(`Transpiling resource by generalize_pam ...`, [`${package_setting.name}:${part_name}/${group_name}/${resource_name}`]);
					let new_source_file = make_scope_child_path(resource_directory, 'source.bin');
			...
			case 'special_wem': {
				...
				if (option.generalize_wem !== null) {
					Console.warning(`Transpiling resource by generalize_wem ...`, [`${package_setting.name}:${part_name}/${group_name}/${resource_name}`]);
					let new_source_file = make_scope_child_path(resource_directory, 'source.bin');
	...
	function transpile_group(
		...
		for (let resource_name of list_scope_child_name(group_directory, resource_scope)) {
			transpile_resource(project_directory, part_name, group_name, resource_name, package_setting, option, buffer);
		}
		return;
	}
	...
	function transpile_part(
		...
		for (let group_name of list_scope_child_name(part_directory, group_scope)) {
			transpile_group(project_directory, part_name, group_name, group_scope.slice(1) as any, package_setting, option, buffer);
		}
		return;
	}
	...
	function transpile_project(
		...
		for (let part_name of list_scope_child_name(project_directory, part_scope)) {
			...
			transpile_part(project_directory, part_name, part_scope.slice(1) as any, package_setting, option, buffer);
		}
		return;
	}
	...
	export function transpile(
		...
		for (let scope of target_scope) {
			transpile_project(project_directory, scope, package_setting, option, buffer);
		}
		return;
	}
