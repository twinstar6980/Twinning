				...
				worker(a: Entry.CFSA & {
					...
					let buffer_size: bigint;
					{
						...
						version_platform = Executor.request_argument(
							...
							(value) => (value),
							null,
							() => (Console.option(CoreX.Tool.PopCap.ReAnimation.VersionPlatformE.map((e) => ([e])), null)),
							(value) => (CoreX.Tool.PopCap.ReAnimation.VersionPlatformE.includes(value as any) ? null : los(`版本不受支持`)),
						);
						version_variant_64 = Executor.request_argument(
							...
							(value) => (CoreX.Tool.PopCap.ReAnimation.VersionVariant64E.includes(value as any) ? null : los(`版本不受支持`)),
						);
