	...
	export function _injector(
				...
				worker(a: Entry.CFSA & {
					...
					let buffer_size: bigint;
					{
						...
						version_number = Argument.request(
							...
							(value) => ([112n].includes(value) ? null : `版本不受支持`),
						);
		...
		g_executor_method_of_batch.push(
			Executor.method_of({
				...
				worker(a: Entry.CFSA & {
					bundle_directory_directory: Argument.Require<string>;
					...
					let buffer_size: bigint;
					{
						bundle_directory_directory = Argument.require(
							'捆绑目录目录', '',
			...
			Executor.method_of({
				...
				worker(a: Entry.CFSA & {
					...
					bundle_directory_directory: Argument.Request<string, true>;
					version_number: Argument.Request<bigint, false>;
				}) {
					let data_file_directory: string;
					let bundle_directory_directory: string;
					let version_number: [112n][number];
					{
						...
						bundle_directory_directory = Argument.request(
							'捆绑目录目录', '',
							a.bundle_directory_directory,
							(value) => (value),
					...
					Console.notify('s', `执行成功`, [`${bundle_directory_directory}`]);
				},
				default_argument: {
					...Entry.k_cfsa,
