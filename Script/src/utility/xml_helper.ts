namespace Twinning.Script.XmlHelper {
	...
	export function wrap_element(
		value: Kernel.Notation.Xml.JS_Element,
	): Kernel.Notation.Xml.JS_ElementNode {
		return {
			type: 'element',
			value: value,
		};
	...
	export function wrap_text(
		value: Kernel.Notation.Xml.JS_Text,
	): Kernel.Notation.Xml.JS_TextNode {
		return {
			type: 'text',
			value: value,
		};
	...
	export function wrap_comment(
		value: Kernel.Notation.Xml.JS_Comment,
	): Kernel.Notation.Xml.JS_CommentNode {
		return {
			type: 'comment',
			value: value,
		};
	...
	export function create_element(
		...
		child: Array<Kernel.Notation.Xml.JS_Node>,
	...
	export function create_text(
		value: string,
		cdata: boolean,
	): Kernel.Notation.Xml.JS_Text {
		return {value, cdata};
	}

	export function create_comment(
		value: string,
	): Kernel.Notation.Xml.JS_Comment {
		return {value};
	}
	...
	export function find_child_element(
		parent: Kernel.Notation.Xml.JS_Element,
		name: string,
	): Array<Kernel.Notation.Xml.JS_Element> {
		return parent.child.filter((child) => (child.type === 'element' && child.value.name === name)).map((child) => (child.value as Kernel.Notation.Xml.JS_Element));
	}
	...
	export function find_child_element_unique_or_none(
		parent: Kernel.Notation.Xml.JS_Element,
		name: string,
	): null | Kernel.Notation.Xml.JS_Element {
		let list = find_child_element(parent, name);
		if (list.length > 1) {
	...
	export function encode_text<TValue extends Kernel.Notation.Xml.JS_Node>(
		value: TValue,
	): string {
	...
	export function decode_text<TValue extends Kernel.Notation.Xml.JS_Node>(
		text: string,
	): TValue {
	...
	export function encode_file<TValue extends Kernel.Notation.Xml.JS_Node>(
		path: StoragePath,
		value: TValue,
	): void {
		let data = Kernel.String.default();
