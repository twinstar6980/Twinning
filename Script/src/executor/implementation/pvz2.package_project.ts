namespace Twinning.Script.Executor.Implementation.PvZ2.PackageProject {
		...
		push_typical_method('pvz2.package_project', [
			typical_method({
				...
				worker: ({
					...
					Support.Pvz2.PackageProject.Transpile.transpile(project_directory, target_scope_list as any, target_package, option, buffer_size.value());
					return;
				},
			}),
			typical_method({
				...
				worker: ({project_directory, target_scope, target_package, buffer_size}, store: {}) => {
					...
					Support.Pvz2.PackageProject.Compile.compile(project_directory, target_scope_list as any, target_package_list, buffer_size.value());
					return;
				},
			}),
			typical_method({
				...
				worker: ({project_directory, target_package, remake_manifest, buffer_size}, store: {}) => {
					let target_package_list = target_package === '*' ? null : target_package.length === 0 ? [] : target_package.split('|');
					Support.Pvz2.PackageProject.Link.link(project_directory, target_package_list, remake_manifest, buffer_size.value());
					return;
				},
			}),
			typical_method({
