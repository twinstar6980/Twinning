namespace Twinning.Script.Support.Popcap.Pvz2.RemoteProject {
	...
	function collect_application_information(
		...
		if (identifier_list.length === 0) {
			Console.warning(los('support.popcap.pvz2.remote_project.execute:application_found_none'), []);
			return null;
		}
		if (identifier_list.length === 1) {
			Console.information(los('support.popcap.pvz2.remote_project.execute:application_found_single'), [identifier_list[0]]);
			identifier = identifier_list[0];
		}
		else {
			Console.information(los('support.popcap.pvz2.remote_project.execute:application_found_multi'), []);
	...
	export function execute(
		...
		Console.information(los('support.popcap.pvz2.remote_project.execute:collect_application_information'), [
		...
		let remote = {
			main_package: remote_application_obb_directory.join(`main.${application.version_code}.${application.identifier}.obb`),
			content_delivery: remote_application_data_directory.join('files').join('No_Backup').join(`CDN.${application.version_name.slice(0, CheckHelper.not_or(application.version_name.lastIndexOf('.'), -1, undefined))}`),
			local_profile: remote_application_data_directory.join('files').join('No_Backup').join('local_profiles'),
		...
		switch (action) {
			...
			case 'check': {
				Console.information(los('support.popcap.pvz2.remote_project.execute:check_path'), [
					los('support.popcap.pvz2.remote_project.execute:main_package', AndroidHelper.fs_exist_file(remote.main_package)),
					los('support.popcap.pvz2.remote_project.execute:content_delivery', AndroidHelper.fs_exist_directory(remote.content_delivery)),
					los('support.popcap.pvz2.remote_project.execute:local_profile', AndroidHelper.fs_exist_file(remote.local_profile)),
					los('support.popcap.pvz2.remote_project.execute:player_profile', AndroidHelper.fs_exist_file(remote.player_profile)),
					los('support.popcap.pvz2.remote_project.execute:player_profile_snapshot_1', AndroidHelper.fs_exist_file(remote.player_profile_snapshot_1)),
					los('support.popcap.pvz2.remote_project.execute:player_profile_snapshot_2', AndroidHelper.fs_exist_file(remote.player_profile_snapshot_2)),
				]);
				break;
			}
			case 'clear': {
			...
			case 'pull': {
				...
				switch (target) {
					case 'main_package': {
						Console.information(los('support.popcap.pvz2.remote_project.execute:pull'), []);
					...
					case 'content_delivery': {
						...
						Console.information(los('support.popcap.pvz2.remote_project.execute:decode'), []);
						for (let sub_file of StorageHelper.list_directory(local_temporary.content_delivery, null, true, false, true, false)) {
							if (sub_file.extension()?.toLowerCase() === 'rton' || sub_file.extension()?.toLowerCase() === 'json') {
								KernelX.Tool.Popcap.ReflectionObjectNotation.decode_fs(local_temporary.content_delivery.push(sub_file), local.content_delivery.push(sub_file.parent()!).join(`${sub_file.stem()}.json`), rton_version);
							}
						}
						break;
					}
			...
			case 'push': {
				...
				switch (target) {
					case 'main_package': {
						Console.information(los('support.popcap.pvz2.remote_project.execute:push'), []);
						AndroidHelper.push(local.main_package, remote.main_package, application.user);
						break;
					}
					case 'content_delivery': {
						Console.information(los('support.popcap.pvz2.remote_project.execute:encode'), []);
						let rton_encode_buffer = Kernel.ByteArray.allocate(Kernel.Size.value(rton_encode_buffer_size));
						for (let sub_file of StorageHelper.list_directory(local.content_delivery, null, true, false, true, false)) {
							KernelX.Tool.Popcap.ReflectionObjectNotation.encode_fs(local_temporary.content_delivery.push(sub_file.parent()!).join(sub_file.name()!.replace(/(?<!(cdn_config|forceupdateconfig))\.json$/i, '.rton')), local.content_delivery.push(sub_file), true, true, rton_version, rton_encode_buffer.view());
						}
						Console.information(los('support.popcap.pvz2.remote_project.execute:push'), []);
