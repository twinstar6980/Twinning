...
namespace Twinning.AssistantPlus.View.CoreModdingWorker {
	...
	public partial class MainPageController : CustomController, Home.IModulePageController {
		...
		public Setting Setting => App.Setting.Data.CoreModdingWorker;
		...
		public async Task OpenView (
			...
			this.Configuration = await JsonHelper.DeserializeFile<Configuration>($"{App.Setting.Data.ModuleConfigurationDirectory}/{ModuleHelper.Query(ModuleType.CoreModdingWorker).Identifier}.json");
			return;
		}
		...
		public async Task<ValueExpression?> ReceiveSubmission (
			...
			var history = App.Setting.State.CoreModdingWorkerSubmissionHistory[type.CastPrimitive<Size>()];
		...
		public async void uAdditionalArgument_Click (
			...
			await ControlHelper.ShowDialogAsAutomatic(this.View, "Additional Argument", new TextBox() {
				...
				FontFamily = this.View.FindResource("CoreModdingWorker.MessageFont").As<FontFamily>(),
	...
	public class MainPageBridgeClient : Bridge.Client {
		...
		private async Task<ValueTuple<String>> CallbackPickStorageItem (
			...
			target = await StorageHelper.Pick(typeValue, App.MainWindow, $"@{nameof(CoreModdingWorker)}.Generic", null) ?? "";
			return new (target);
		}
