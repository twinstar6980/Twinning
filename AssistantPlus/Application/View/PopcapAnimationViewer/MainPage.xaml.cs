...
namespace Twinning.AssistantPlus.View.PopcapAnimationViewer {
	...
	public partial class MainPageController : CustomController, Home.IModulePageController {
		...
		public PopcapAnimationModel.Animation? Animation { get; set; } = null;
		...
		public PopcapAnimationModel.Sprite? ActiveSprite { get; set; } = null;

		public List<Tuple<String, PopcapAnimationHelper.FrameRange>>? ActiveFrameLabel { get; set; } = null;

		public PopcapAnimationHelper.FrameRange? ActiveFrameRange { get; set; } = null;
		...
		public async Task Activate (
			...
			if (!target.Item1) {
				var originalTarget = PopcapAnimationHelper.SelectImage(this.Animation, target.Item2.CastPrimitive<Size>());
				activeSprite = new () {
					Name = PopcapAnimationHelper.ParseImageFileName(originalTarget.Name),
					FrameRate = null,
					WorkArea = new (0, 0),
					Frame = [
						new () {
			...
			this.ActiveFrameLabel = PopcapAnimationHelper.ParseSpriteFrameLabel(this.ActiveSprite);
		...
		public async void uActiveFrameRangeBegin_ValueChanged (
			...
			if (Floater.IsFinite(args.NewValue)) {
				...
				var newRange = new PopcapAnimationHelper.FrameRange() {
					Begin = newBegin,
	...
	public partial class MainPageImageItemController : CustomController {
		...
		public String uTitle_ToolTip {
			get {
				...
				return PopcapAnimationHelper.ParseImageFileName(model.Name);
			}
		}
		...
		public UIElement uPreview_Content {
			get {
				...
				if (model.Frame.Count == 1 && model.Frame[0].Append.Count == 1 && model.Frame[0].Change.Count == 1 && !model.Frame[0].Append[0].Sprite) {
					texture = this.Host.Texture.GetValueOrDefault(PopcapAnimationHelper.SelectImage(this.Host.Animation, model.Frame[0].Append[0].Resource.CastPrimitive<Size>()).Name);
				}
				if (texture != null) {
					return new Image() {
						Width = 32, Height = 32,
