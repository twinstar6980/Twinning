...
namespace TwinKleS::Core::XML {
	...
	inline namespace CommonOfWrite {
		...
		namespace Detail {
			...
			inline auto move_printer_buffer_if_can (
	...
	namespace Write {
		...
		namespace Detail {
				...
				switch (node.type()) {
					case NodeType::element : {
						...
						for (auto & child : node_value.child) {
							raw_element->InsertEndChild(cast_node(raw_document, child));
						}
						break;
					}
					case NodeType::text : {
			...
			inline auto process (
				...
				raw_document.InsertEndChild(cast_node(raw_document, node));
				auto printer = TinyXML2Printer{};
				raw_document.Print(&printer);
				return move_printer_buffer_if_can(printer);
			}
	...
	namespace Read {
		...
		namespace Detail {
				...
				if (auto raw_element = raw_node->ToElement()) {
					...
					}
					{
						...
						for (auto raw_child = raw_element->FirstChild(); raw_child; raw_child = raw_child->NextSibling()) {
							...
							cast_node(raw_child, node_value.child.last());
						}
					}
			...
			inline auto process (
				CCharacterListView const & string
			) -> Node {
				auto node = Node{};
				auto raw_document = ThirdParty::tinyxml2::XMLDocument{};
				raw_document.Parse(cast_pointer<ZCharacter>(string.begin()).value, string.size().value);
				cast_document(raw_document, node);
				return node;
			}
