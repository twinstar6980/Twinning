...
namespace TwinStar::Core::Tool::PopCap::ResourceStreamBundle {
	...
	struct Pack :
		...
		static auto process_package_description (
			...
			typename Description::Package const & description
		) -> Void {
			...
			set_string(""_s);
			group_description_information_structure_list.allocate_full(description.group.size());
			...
			for (auto & group_index : SizeRange{description.group.size()}) {
				auto & group_description = description.group.at(group_index);
			...
			header_structure.description_group_section_offset = cbw<IntegerU32>(data.position());
			data.write(group_description_information_structure_list);
			header_structure.description_resource_section_offset = cbw<IntegerU32>(data.position());
			data.write(resource_detail_description_information_structure_list);
			header_structure.description_string_section_offset = cbw<IntegerU32>(data.position());
		...
		static auto process_package (
			...
			Optional<typename Description::Package> const & description,
				...
				information_data = {};
			{
				...
				information_data.subgroup_pool_information_offset = data.position();
				information_data.subgroup_pool_information = OByteStreamView{
					data.forward_view(bs_size(information_structure.subgroup_pool_information))
				};
			...
			if constexpr (check_version(version, {4}, {})) {
				...
				information_structure.header.information_without_description_section_size = cbw<IntegerU32>(data.position());
			}
			if (description.has()) {
				process_package_description(data, information_structure.header, description.get());
			} else {
			...
			for (auto & group_index : SizeRange{definition.group.size()}) {
				...
				for (auto & subgroup_index : SizeRange{group_definition.value.subgroup.size()}) {
					...
					subgroup_pool_information_structure.id = string_block_fixed_128_from_string(subgroup_definition.key + k_suffix_of_automation_pool);
						...
						switch (resource_definition.value.additional.type().value) {
							...
							case ResourceType::Constant::texture().value : {
								...
								if constexpr (check_version(version, {4}, {2})) {
									texture_information_structure.scale = cbw<IntegerU32>(resource_additional_definition.scale);
								}
								packet_resource_additional_definition.index = cbw<Integer>(texture_resource_count);
		...
		static auto process (
			...
			M_use_zps_of(data);
			return process_package(data, definition, description, resource_directory, packet_file, new_packet_file);
		}
