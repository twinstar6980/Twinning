...
#include "core/tool/popcap/re_animation/common.hpp"
...
namespace TwinStar::Core::Tool::PopCap::ReAnimation {
	...
	struct Decode :
		...
		static auto exchange_transform_list (
			IByteStreamView &                      data,
			List<typename Definition::Transform> & value_list
		) -> Void {
			auto ignored = Integer{0_i};
			exchange_raw_constant(data, cbw<IntegerU32>(k_transform_data_size));
		...
		static auto exchange_track_list (
			IByteStreamView &                  data,
			List<typename Definition::Track> & value_list
		) -> Void {
			auto ignored = Integer{0_i};
			exchange_raw_constant(data, cbw<IntegerU32>(k_track_data_size));
			for (auto & value : value_list) {
				exchange_integer_fixed<IntegerOfPlatform>(data, ignored);
				exchange_integer_fixed<IntegerOfPlatform>(data, ignored);
