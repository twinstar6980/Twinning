...
#include "core/tool/wwise/sound_bank/common.hpp"
				...
				exchange_list(
					...
					[] (auto & data, auto & value) {
						...
						if (!has_case) {
							...
							type = cbw<Enumerated>(unknown_value.type);
							item_data.write(unknown_value.data);
						}
						exchange_enumerated_fixed<IntegerU8>(type_data, type);
						exchange_integer_fixed<IntegerU32>(size_data, cbw<Integer>(item_data.position()));
