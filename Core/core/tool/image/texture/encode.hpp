...
namespace TwinStar::Core::Tool::Image::Texture {
	...
	protected:
		...
		static auto process_pixel (
			...
			if constexpr (format == Format::Constant::a_8()) {
				data.write(cbw<IntegerU8>(pixel.alpha));
			}
			if constexpr (format == Format::Constant::rgb_888()) {
				data.write(cbw<IntegerU8>(pixel.red));
				data.write(cbw<IntegerU8>(pixel.green));
				data.write(cbw<IntegerU8>(pixel.blue));
			}
	...
	protected:
		...
		static auto process_pixel (
			...
			if constexpr (format == Format::Constant::a_8()) {
				pixel.alpha = cbw<Image::Color>(data.read_of<IntegerU8>());
			}
			if constexpr (format == Format::Constant::rgb_888()) {
				pixel.red = cbw<Image::Color>(data.read_of<IntegerU8>());
				pixel.green = cbw<Image::Color>(data.read_of<IntegerU8>());
				pixel.blue = cbw<Image::Color>(data.read_of<IntegerU8>());
			}
			if constexpr (format == Format::Constant::rgba_8888()) {
				pixel.red = cbw<Image::Color>(data.read_of<IntegerU8>());
				pixel.green = cbw<Image::Color>(data.read_of<IntegerU8>());
