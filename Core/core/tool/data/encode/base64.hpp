...
namespace TwinKleS::Core::Tool::Data::Encode::Base64 {
	...
	inline namespace Common {
		...
		namespace Detail {
			...
			inline constexpr auto k_raw_block_size = Size{3_sz};

			inline constexpr auto k_ripe_block_size = Size{4_sz};

			inline constexpr auto k_padding_character = Character{'='_c};
	...
	namespace Encode {
		...
		namespace Detail {
			...
			inline constexpr auto k_table = StaticArray<Character, 64_szz>{{
				// 'A' - 'Z'
			...
			inline auto compute_size (
				...
				return compute_padded_size(raw_size, k_raw_block_size) / k_raw_block_size * k_ripe_block_size;
			}
	...
	namespace Decode {
		...
		namespace Detail {
			...
			inline constexpr auto k_table = StaticArray<Byte, 128_szz>{{
				// none
			...
			inline auto compute_size (
				...
				return ripe.size() / k_ripe_block_size * k_raw_block_size - padding_size;
			}
			...
			inline auto process (
				...
				auto raw_end = raw.position() + compute_size(ripe.reserve_view());
