...
namespace TwinKleS::Core::Tool::ProtocolBufferVariableLengthInteger {
	...
	namespace ZigZag {
		...
		inline auto encode_32 (
			...
			return cbw<IntegerU32>(value << 1_sz ^ value >> 31_sz);
		}

		inline auto encode_64 (
			IntegerS64 const & value
		) -> IntegerU64 {
			return cbw<IntegerU64>(value << 1_sz ^ value >> 63_sz);
		}
		...
		inline auto decode_32 (
			...
			return cbw<IntegerS32>(value >> 1_sz) ^ -cbw<IntegerS32>(value & 1_iu32);
		}

		inline auto decode_64 (
			IntegerU64 const & value
		) -> IntegerS64 {
			return cbw<IntegerS64>(value >> 1_sz) ^ -cbw<IntegerS64>(value & 1_iu64);
		}
	...
	inline auto encode_u32 (
		...
		while (value >= cbw<IntegerU32>(k_b)) {
			stream.write(cbw<Byte>(value) & k_x | k_b);
			value >>= 7_sz;
		}
		stream.write(cbw<Byte>(value));
		return;
	}
	...
	inline auto encode_u64 (
		...
		while (value >= cbw<IntegerU64>(k_b)) {
			stream.write(cbw<Byte>(value) & k_x | k_b);
			value >>= 7_sz;
		}
		stream.write(cbw<Byte>(value));
