...
extension SettingListItemExtension on SettingListItem {
  ...
  static SettingListItem buildForStoragePathVariable({
    ...
    required Future<Boolean> Function(StoragePath value) checkValue,
    required String                                      pickerTag,
    required List<StoragePickType>                       pickerType,
    required Void Function()                             onUpdate,
  }) {
      ...
      comment: [
        FutureBuilder(
          ...
          builder: (context, snapshot) => snapshot.connectionState != .done
            ...
            : StyledText.inherit(snapshot.hasError ? 'Error' : !snapshot.data! ? comment.negative : comment.positive)
        ),
      ],
      onPressed: null,
      panelBuilder: (context, setStateForPanel) => [
