...
namespace Twinning::Kernel::Tool::PopCap::ParticleEffect {
	...
	struct Decode :
		...
		static auto process_value_1 (
			...
			count = cbw<Size>(count_with_flag & 0b111_i);
			if (count == 0b111_sz) {
				exchange_size_fixed<IntegerS16>(data, count);
			}
			flag.from_integer(cbw<IntegerU8>(count_with_flag >> 3_sz));
		...
		static auto process_value_2_simple (
			IByteStreamView &             data,
			typename Definition::Value2 & value
		) -> Void {
