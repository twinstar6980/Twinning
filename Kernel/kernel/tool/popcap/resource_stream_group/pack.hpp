		...
		static auto process_package (
				...
				information_data = {};
			{
				...
				for (auto & resource_index : SizeRange{definition.resource.size()}) {
					...
					resource_information_structure.key = resource_definition.key.to_string(CharacterType::PathSeparator::windows);
			...
			information_structure.header.resource_data_section_store_mode = resource_data_section_store_mode_to_data(definition.resource_data_section_store_mode);
			information_structure.resource_information.allocate_full(definition.resource.size());
			for (auto & current_resource_type : make_static_array<ResourceType>(ResourceType::Constant::generic(), ResourceType::Constant::texture())) {
				...
				for (auto & resource_index : SizeRange{definition.resource.size()}) {
					auto & resource_definition = definition.resource.at(resource_index);
					if (resource_definition.value.additional.type() != current_resource_type) {
						continue;
					}
					auto resource_path_full = resource_directory / resource_definition.key;
