...
namespace TwinStar::Kernel::Tool::PopCap::ResourceStreamBundle {
	...
	struct Pack :
		...
		static auto process_package_manifest (
			...
			for (auto & group_index : SizeRange{manifest.group.size()}) {
				...
				for (auto & subgroup_index : SizeRange{group_manifest.subgroup.size()}) {
					...
					for (auto & resource_index : SizeRange{subgroup_manifest.resource.size()}) {
						...
						resource_detail_manifest_information_structure.property_information_count = cbw<IntegerU32>(resource_detail_manifest_information_structure.property_information.size());
			...
			header_structure.group_manifest_information_section_offset = cbw<IntegerU32>(data.position());
			data.write(group_manifest_information_structure_list);
			header_structure.resource_manifest_information_section_offset = cbw<IntegerU32>(data.position());
			data.write(resource_detail_manifest_information_structure_list);
			header_structure.string_manifest_information_section_offset = cbw<IntegerU32>(data.position());
		...
		static auto process_package (
			...
			information_structure.header.pool_information_section_block_count = cbw<IntegerU32>(global_subgroup_count);
			information_structure.header.pool_information_section_offset = cbw<IntegerU32>(information_data.pool_information_offset);
			information_structure.header.pool_information_section_block_size = cbw<IntegerU32>(bs_static_size<Structure::PoolInformation<version>>());
			...
			if constexpr (check_version(version, {4}, {})) {
				...
				information_structure.header.information_without_manifest_section_size = cbw<IntegerU32>(data.position());
			}
			if (manifest.has()) {
			...
			for (auto & group_index : SizeRange{definition.group.size()}) {
				...
				for (auto & subgroup_index : SizeRange{group_definition.subgroup.size()}) {
					...
					if constexpr (check_version(version, {1, 3}, {})) {
						...
						pool_information_structure.texture_resource_count = cbw<IntegerU32>(texture_resource_count);
					}
					if constexpr (check_version(version, {3}, {})) {
