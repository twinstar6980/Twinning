...
#include "kernel/tool/popcap/sexy_texture/common.hpp"
...
namespace TwinStar::Kernel::Tool::PopCap::SexyTexture {
	...
	struct Decode :
		...
		static auto process_image (
			...
			auto header = Header{};
			data.read(header);
			assert_test(image.size() == Image::ImageSize{cbw<Size>(header.size_width), cbw<Size>(header.size_height)});
			auto image_format = cbw<Integer>(header.format);
			...
			auto compress_texture_data = cbw<Boolean>(header.compress_data);
			auto compress_texture_data_size = cbw<Size>(header.compress_data_size);
		...
		static auto estimate_image (
			...
			image_size = Image::ImageSize{cbw<Size>(header.size_width), cbw<Size>(header.size_height)};
			return;
		}
