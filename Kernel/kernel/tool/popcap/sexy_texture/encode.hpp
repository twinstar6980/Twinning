...
namespace TwinStar::Kernel::Tool::PopCap::SexyTexture {
	...
	struct Encode :
		...
		static auto process_image (
			...
			auto header = Header{};
			header.size_width = cbw<IntegerU32>(image.size().width);
			header.size_height = cbw<IntegerU32>(image.size().height);
			header.format = cbw<IntegerU32>(image_format);
			header.compress_data = cbw<IntegerU32>(compress_texture_data);
			header.compress_data_size = cbw<IntegerU32>(compress_texture_data_size);
