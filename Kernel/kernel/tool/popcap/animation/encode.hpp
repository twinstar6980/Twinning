...
#include "kernel/tool/popcap/animation/common.hpp"
...
namespace TwinStar::Kernel::Tool::PopCap::Animation {
	...
	struct Encode :
		...
		template <typename RawShortValue, typename RawLongValue> requires
			...
			&& (IsIntegerWrapper<RawShortValue>)
			&& (IsIntegerWrapper<RawLongValue>)
		static auto exchange_size_variant (
			OByteStreamView & data,
			Size const &      value
		) -> Void {
			auto value_integer = cbw<Integer>(value);
		...
		template <typename RawShortValue, typename RawLongValue> requires
			...
			&& (IsIntegerWrapper<RawShortValue>)
			&& (IsIntegerWrapper<RawLongValue>)
		static auto exchange_integer_variant (
			OByteStreamView & data,
			Integer const &   value
		) -> Void {
			auto value_short_maximum = ~mbw<RawShortValue>(0);
		...
		static auto exchange_integer_variant_with_flag (
			OByteStreamView &          data,
			Integer const &            value,
			BitSet<flag_count> const & flag
		) -> Void {
			auto value_short_bit_count = k_type_bit_count<RawShortValue> - flag_count;
			auto value_short_maximum = ~(~mbw<RawShortValue>(0) << value_short_bit_count);
			auto value_long = cbw<RawLongValue>(value);
			auto value_flag = cbw<RawShortValue>(flag.to_integer());
		...
		static auto exchange_floater_with_rate (
			OByteStreamView & data,
			Floater const &   value
		) -> Void {
		...
		static auto exchange_layer_remove (
			OByteStreamView &                        data,
		...
		static auto exchange_layer_append (
			OByteStreamView &                        data,
		...
		static auto exchange_layer_change (
			OByteStreamView &                        data,
		...
		static auto exchange_frame (
			...
			exchange_integer_fixed<IntegerU8>(data, cbw<Integer>(flag.to_integer()));
		...
		static auto exchange_animation (
			OByteStreamView &                      data,
