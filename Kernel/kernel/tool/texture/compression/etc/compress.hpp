...
#include "kernel/tool/texture/compression/etc/common.hpp"
...
namespace Twinning::Kernel::Tool::Texture::Compression::ETC {
	...
	struct Compress :
		...
		static auto process_image_v1_rgb (
		...
		static auto process_image_v2_rgb (
			OByteStreamView &         data,
			Image::CImageView const & image
		) -> Void {
			assert_test(is_padded_size(image.size().width, k_block_width));
		...
		static auto process_image_v2_rgba (
			OByteStreamView &         data,
			Image::CImageView const & image
		) -> Void {
			assert_test(is_padded_size(image.size().width, k_block_width));
